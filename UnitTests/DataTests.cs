...
namespace UnitTests
{
    ...
    public class DataTests : IDisposable
    {
        ...
        public void TestEmptyMessageDoesNotDisconnectClient()
        {
            ...
            _actualHash.Should().NotBeNullOrEmpty("Server should have received a zero-byte message from the client");
            _expectedHash.Should().BeEquivalentTo(_actualHash, "SHA-1 hashes for zero-byte message should match");
            _clientDisconnected.Should().BeFalse("Server should not disconnect the client for explicitly sending zero-length data");
        }
        ...
        public void TestMessageSize1B()
        {
            ...
            _actualHash.Should().NotBeNull(string.Format("Server should have received client's {0} byte message", numBytes));
            _expectedHash.Should().BeEquivalentTo(_actualHash, string.Format("SHA-1 hashes for {0} byte message should match", numBytes));
            _clientDisconnected.Should().BeFalse("Server should still be connected to the client");
        }
