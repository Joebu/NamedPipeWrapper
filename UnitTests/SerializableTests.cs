...
namespace UnitTests
{
    ...
    public class SerializableTests : IDisposable
    {
        ...
        public void TestCircularReferences()
        {
            ...
            //_actualHash.Should()(string.Format("Server should have received client's {0} item message", _expectedData.Count));
            _expectedHash.Should().Be(_actualHash, string.Format("Hash codes for {0} item message should match", _expectedData.Count));
            _expectedData.Count.Should().Be(_actualData.Count, string.Format("Collection lengths should be equal"));
            
            for (var i = 0; i < _actualData.Count; i++)
            {
                var expectedItem = _expectedData[i];
                var actualItem = _actualData[i];
                expectedItem.Should().BeEquivalentTo(actualItem, string.Format("Items at index {0} should be equal", i));
                actualItem.Parent.Should().BeEquivalentTo( _actualData, string.Format("Item at index {0}'s Parent property should reference the item's parent collection", i));
            }
        }
